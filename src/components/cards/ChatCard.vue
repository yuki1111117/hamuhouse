  ...
  <v-card elevation="0" class="cardContainer" color="background">
    <v-dialog v-model="dialog" max-width="500px">
      <v-container>
        <v-row>
          ...
          <v-col cols="12" sm="6" md="4">
            <v-text-field v-model="editedItem.good" type="number" label="Good"> </v-text-field>
          </v-col>
          <v-col cols="12" sm="6" md="4">
            <v-text-field v-model="editedItem.chat" label="Chat"> </v-text-field>
          </v-col>
          <v-col cols="12" sm="6" md="4">
    ...
    <div class="cardAvator">
      <v-card-text class="cardRankingInfo text--disabled">
        {{ index + 1 }}
        <v-checkbox v-model="editedItem.done" @click="check(item)"> </v-checkbox>
      </v-card-text>
      <v-avatar size="24px">
    ...
    <div class="cardText">
      <div class="cardInfoTop d-flex justify-space-between">
        ...
        <v-card-text class="cardInfo text--disabled"> <v-spacer></v-spacer>{{ item.time }} </v-card-text>
      </div>
      ...
      <v-row justify="end">
        <v-card-actions class="text--disabled">
          <v-icon class="iconBtn" size="12px" color="fontcolor" @click="editItem(item)"> mdi mdi-pencil </v-icon>
          <v-icon class="iconBtn" size="12px" color="fontcolor" @click="deleteMessage(item)"> mdi mdi-eraser </v-icon>
          <v-icon size="12px" color="fontcolor" @click="goodMessage(item)"> mdi-heart </v-icon>
